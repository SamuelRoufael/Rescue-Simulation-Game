...
public class ResidentialBuilding implements Rescuable, Simulatable 
{
	...
	public String toString() {
		String s = "";
		s+= "Building structuralIntegrity : " + structuralIntegrity + "\n";
		s+= "Building location : " + location +"\n";
		s+= "Building fireDamage : " + fireDamage +"\n";
		s+= "Building gasLevel : " + gasLevel +"\n";
		s+= "Building foundationDamage :" + foundationDamage +"\n";
		s+= "Building Disaster : ";
		...
		if(disaster instanceof Collapse)
			s+= "Collapse \n";
		s+= "-----Citzens in Building----- \n";
		for(int i=0;i<occupants.size();i++) {
			s+= "---citizen "+(i+1)+"--- \n";
			s+= occupants.get(i).toString()+"\n";
		}
		return s;
	}
